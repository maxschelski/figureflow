...
def add_column_plot_title_above(ax, col_val, col_label_padding, fontsize):
    fig = plt.gcf()
    # set title and move plot down (by decreasing height) accordingly right away
    title = ax.set_title(col_val, pad = col_label_padding, fontsize=fontsize)
...
def get_width_of_legend(legend, borderaxespad_px, fig):
...
def points_to_pixel(points):
    ...
    return 10 * points * plt.gcf().dpi / 72
...
def set_legend_and_axes(ax, col_order, plot_nb, hue_order,
    ...
    borderaxespad_px = points_to_pixel(borderaxespad_)
    ...
    if type(legend) != type(None):
        ...
        legend_width = get_width_of_legend(legend, borderaxespad_px, fig)
...
def plot_and_add_stat_annotation(data=None, x=None, y=None, hue=None, x_order=[],
    ...
    for col_nb, col_val in enumerate(col_order):
        ...
        if (plot_type not in continuous_plot_types):
            if (hue != "no_hue_defined"):
        ...
        legend_width, longest_legend_handles = set_legend_and_axes(ax, col_order, plot_nb, hue_order,
                            y_axis_label, x_order, fontsize, legend_spacing, longest_legend_handles,
